...
export const placeOrderService = async (userId, cartItems) => {
  ...
  try {
    ...
    const result = await client.query("SELECT id, price FROM products WHERE id = ANY($1);", [productIds])

    const priceMap = Object.fromEntries(result.rows.map(p => [p.id, p.price]))
    ...
    //insert order
    const orderResult = await client.query('INSERT INTO orders (user_id, total_price) VALUES ($1, $2) RETURNING *;', [userId, totalPrice])
    const orderId = orderResult.rows[0].id

    //insert order items
    for(const item of cartItems) {
      await client.query('INSERT INTO order_items (order_id, product_id, quantity, unit_price) VALUES ($1, $2, $3, $4)', [orderId, item.product_id, item.quantity, priceMap[item.product_id]])
    }
...
export const getPaginatedOrdersByUserService = async (page = 1, limit = 10, userId) => {
  const offset = (page - 1) * limit;
    ...
    WHERE o.user_id = $1
    GROUP BY o.id, u.username, u.role
    ...
    const {rows} = await query('SELECT COUNT(*) FROM orders WHERE user_id = $1;', [userId])
...
export const getAllPaginatedOrdersService = async (page = 1, limit = 10) => {
  ...
  const queryTxt = `
    ...
    JOIN users u ON o.user_id = u.id
    LEFT JOIN order_items oi ON o.id = oi.order_id
    GROUP BY o.id, u.fullname, u.role, u.active
    ORDER BY o.created_at DESC
    LIMIT $1 OFFSET $2;
    `;
...
export const getRecentOrdersService = async () => {
  const queryTxt = `
    ...
    JOIN users u ON o.user_id = u.id
    LEFT JOIN order_items oi ON o.id = oi.order_id
    GROUP BY o.id, u.fullname, u.role
    ORDER BY o.created_at DESC
    LIMIT 10;
    `;
...
WHERE oi.order_id = $1;
  ...
  return {orderSummary: orderSummaryResult.rows, totalPrice: totalPriceResult.rows}
}
